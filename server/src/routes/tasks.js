...
// GET /api/tasks?boardId=&column=&sprint=&assignee=&search=&page=&limit=
...
router.post("/:id/comments", protect, async (req, res, next) => {
  try {
    ...
    res.json({ comment: task.comments.at(-1) });
  } catch (err) {
    next(err);
  }
});

// PATCH /api/tasks/reorder — bulk update
